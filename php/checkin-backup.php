...
// This is the racer check-in page.  It appears as a table of all the registered racers, with a check-in
// button (actually, checkbox) for each racer.  Clicking on the check-in button invokes some
// javascript that sends an ajax POST request to check-in (or un-check-in) that racer.  See checkin.js.

// In addition to the actual check-in, it's possible to change a racer's car number from this form, or 
// mark the racer for our "exclusively by scout" award.

// Here on the server side, a GET request sends HTML for the whole page, while a POST returns just
// a small XML document.

// TODO- subgroups explanation

// $use_subgroups, from GPRM settings, tells whether we're using "subgroups"
// within each racing group.
$use_subgroups = read_raceinfo_boolean('use-subgroups');

// Our pack provides an "exclusively by scout" award, based on a signed statement from the parent.
// Collecting the statement is part of the check-in process, so there's provision for a checkbox
// on the check-in form.  For groups that don't do this, $xbs will be false (and $xbs_award_name 
// will be blank), and the checkboxes won't be shown.
...
if (!$order)
  ...
  // The table of racers can be presented in order by name, car, or den (and then by name within the den).
  // Each sortable column has a header which is a link to change the ordering, with the exception that
  // the header for the column for ordering currently in use is NOT a link (because it wouldn't do anything).
  ...
  if (odbc_result($rs, 'Scheduled')) {
    echo '<td>'.($passed ? 'Racing' : 'Scheduled but not passed').'</td>';
      ...
      echo '<td class="clickable">'
	      .'<input type="checkbox" name="passed-'.$racer_id.'" id="passed-'.$racer_id.'"'
    ...
    if ($xbs) {
        echo '<td class="clickable">'.'<input type="checkbox" name="xbs-'.$racer_id.'" '
...
<?php if (have_permission(REGISTER_NEW_RACER_PERMISSION)) { ?>
    <div class="block_buttons">
	 <input type="button" value="New Racer" onclick="window.location = 'newracer.php';"/>
	</div>
<?php } ?>

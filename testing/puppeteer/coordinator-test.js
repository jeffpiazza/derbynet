...
// TODO  page.on('console', msg => { console.log(msg); });
  ...
  await page.evaluate(function(xml) { process_coordinator_poll_response((new DOMParser()).parseFromString(xml, 'text/xml')); },
                ...
                '<coordinator_poll>\n' +
                '  <current-heat now-racing="1" use-master-sched="0" classid="3" roundid="3" round="1" tbodyid="3" heat="3" number-of-heats="4">Bears and Frèr</current-heat>\n' +
  ...
  var arrows_group = await page.$(".control_group[data-roundid='5']");
  arrows_group.click();

  // Wait for control group to open
  await page.waitFor(() => { return $(".control_group[data-roundid='5']").height() > 200; });

  // Click Schedule button to open schedule modal
  
  var arrows_schedule_button =
      await page.waitFor(".control_group[data-roundid='5'] input[type='button'][value='Schedule']");
  arrows_schedule_button.click();
  ...
  // For whatever reason, the master scheduling control may not respond to clicks right away.
  // Keep clicking until the control starts to change size
  await page.evaluate(() => {
    var control = $("#master-schedule-group .scheduling_control");
    var before_height = control.height();
    control.click();
    var interval = setInterval(function() {
      if (control.height() > before_height) {
  ...
  // Wait for control group to open
  await page.waitFor(() => { return $("#master-schedule-group .scheduling_control").height() > 200; });

...
# Voting not yet open
curl_postj action.php "action=json.vote.cast&awardid=3&votes=[12,36]" | check_jfailure

curl_postj action.php "action=json.ballot.open&state=open" | check_jsuccess

curl_postj action.php "action=json.vote.cast&awardid=3&votes=[16,42]" | check_jsuccess
# The same voter selecting different candidates doesn't add to the original choices
curl_postj action.php "action=json.vote.cast&awardid=3&votes=[13,37]" | check_jsuccess
curl_postj action.php "action=json.vote.cast&awardid=1&votes=[11,12]" | check_jsuccess

curl_postj action.php "action=json.vote.cast&awardid=4&votes=[12,13]" | check_jfailure # Too many choices

# Delete the session cookie in order to change voters
rm $COOKIES_CURL

curl_postj action.php "action=json.vote.cast&awardid=3&votes=[19,37]" | check_jsuccess
...
curl_postj action.php "action=json.vote.cast&awardid=3&votes=[36]" | check_jsuccess
curl_getj "action.php?query=json.ballot.results" | \
    jq '.awards | any( .candidates |  any( .carnumber == 136 ))' | \
    expect_eq true

curl_postj action.php "action=json.vote.cast&awardid=3&votes=[42]" | check_jsuccess
curl_getj "action.php?query=json.ballot.results" | \
    jq '.awards | any( .candidates |  any( .carnumber == 242 ))' | \
    expect_eq true
